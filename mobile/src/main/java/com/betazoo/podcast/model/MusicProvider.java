...
import static com.betazoo.podcast.utils.MediaIDHelper.MEDIA_ID_MUSICS_BY_GENRE;
import static com.betazoo.podcast.utils.MediaIDHelper.MEDIA_ID_ROOT;
import static com.betazoo.podcast.utils.MediaIDHelper.createMediaID;
...
public class MusicProvider {
    ...
    private final ConcurrentMap<String, MutableMediaMetadata> mCategoryListById;
    ...
    private volatile  CategoryState mCategoryCurrentState = CategoryState.NON_INITIALIZED;
    ...
    public MusicProvider(MusicProviderSource source) {
        ...
        mCategoryListById = new ConcurrentHashMap<>();
     ...
     * Get an iterator over a shuffled collection of all songs
     */
    public Iterable<MediaMetadataCompat> getAllPodcast() {
        if (mCurrentState != State.INITIALIZED) {
            return Collections.emptyList();
        }
        List<MediaMetadataCompat> shuffled = new ArrayList<>(mMusicListById.size());
    ...
    private synchronized void retrievePodcast(Context context) {
        try {
            if (mCurrentState == State.NON_INITIALIZED) {
                mCurrentState = State.INITIALIZING;

                Iterator<MediaMetadataCompat> tracks = mSource.podcastIterator(context);
                while (tracks.hasNext()) {
                    MediaMetadataCompat item = tracks.next();
                    String musicId = item.getString(MediaMetadataCompat.METADATA_KEY_MEDIA_ID);
                    mMusicListById.put(musicId, new MutableMediaMetadata(musicId, item));
    ...
    private MediaBrowserCompat.MediaItem createBrowsableMediaItemForRoot(Resources resources) {
        MediaDescriptionCompat description = new MediaDescriptionCompat.Builder()
                ...
                .setIconUri(Uri.parse("android.resource://" +
                        "com.betazoo.podcast/drawable/ic_by_genre"))
                .build();
